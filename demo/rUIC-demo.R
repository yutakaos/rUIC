...
g8 <- ggplot(uic_yx, aes(x = tp, y = te)) +
    ...
    scale_x_continuous(breaks = -4:5) + ylim(-0.01, 1.6) +
    xlab("tp") + ylab("UIC") + ggtitle(expression("UIC (x cause y?)")) +
    theme(legend.position = "none")

# Visualize marginal UIC
g9 <- ggplot(muic_xy, aes(x = tp, y = te)) +
    geom_line() + geom_point(aes(color = pval < 0.05), size = 2) +
    scale_color_manual(values = c("black", "red3")) +
    scale_x_continuous(breaks = -4:5) + ylim(-0.01, 0.4) +
    xlab("tp") + ylab("UIC") + ggtitle(expression("marginal UIC (y cause x?)")) +
    theme(legend.position = "none")
g10 <- ggplot(muic_yx, aes(x = tp, y = te)) +
    geom_vline(xintercept = -1, size = 3, alpha = 0.2) +
    geom_line() + geom_point(aes(color = pval < 0.05), size = 2) +
    scale_color_manual(values = c("black", "red3")) +
    scale_x_continuous(breaks = -4:5) + ylim(-0.01, 0.4) +
...
ggsave("demo_figures/muic.png",
       plot_grid(g9, g10, nrow = 1, align = "hv"),
       width = 8, height = 3.5)
