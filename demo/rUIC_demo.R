...
ts <- ggplot(block, aes(x = t, y = x)) + geom_line(color = "royalblue") +
  geom_line(data = block, aes(x = t, y = y), color = "red3") +
  ggtitle("Model time series") + ylab("X (blue) or Y (red)") + xlab("Time") + xlim(100,200)
...
ggsave("demo_figures/simplex_uic.png",
