/*****************************************************************************
 * Brute-force Nearest Neighbor Seach for UIC
 * 
...
public:
    ...
    inline size_t search (
        ...
        const data_t &query,
        const time_t &qtime, // time indices of query
        size_t num_nearest,
        bool tied = true) const
    {
        //* compute distance */
        std::vector<num_t> ds(nt);
        for (size_t i = 0; i < nt; ++i) ds[i] = eval_norm(query, i, n_dim);
        //for (size_t i = 0; i < nt; ++i) ds[i] = eval_norm(&query[0], i, n_dim);
